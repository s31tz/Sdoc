...
Sdoc::Core::FileHandle - Datei-Handle
...
Datei schreiben:
  ...
  my $fh = Sdoc::Core::FileHandle->new('>',$path);
  $fh->print("Test\n");
  $fh->close;
...
Programm darf nur einmal laufen:
  ...
  my $fh = eval{Sdoc::Core::FileHandle->new('<',$file,-lock=>'EXNB')};
  if ($@) {
      exit 5;
  }
...
Filehandle-Objekt für STDOUT:
  ...
  $fh = Sdoc::Core::FileHandle->new(\*STDOUT);
...
Liefere Chunks von 1000 Pfaden:
  ...
  my $fh = Sdoc::Core::FileHandle->new('-|',"find @$dirA -name '*.xml.gz'");
...
Lies gesamten Inhalt einer Datei ab Position $pos:
  ...
  my $fh = Sdoc::Core::FileHandle->open('<',$logFile);
  $fh->seek($pos);
  my $data = $fh->slurp;
