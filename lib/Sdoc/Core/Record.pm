...
Sdoc::Core::Record - Verarbeitung von Text-Records
...
sub fromString {
    ...
    for (my $i = 0; $i < @keys; $i++) {
        $arr[$i*2] = $keys[$i];
        my $val = Sdoc::Core::String->removeIndentation($vals[$i]);
        $arr[$i*2+1] = $val;
    }
...
sub fromFile {
    ...
    return $class->fromString(Sdoc::Core::Path->read($file,-decode=>$encoding));
}
...
sub toString {
    ...
    for (my $i = 0; $i < @$arr; $i += 2) {
        ...
        if ($format eq ':' && $indent) {
            Sdoc::Core::String->indent(\$val,$indent);
        }
        if ($space && $str) {
            $str .= $space;
        }
...
sub toFile {
    ...
    Sdoc::Core::Path->write($file,$class->toString(@_));    
    return;
}
