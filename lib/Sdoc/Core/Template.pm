...
Sdoc::Core::Template - Klasse für HTML/XML/Text-Generierung
...
Code:
  ...
  $tpl = Sdoc::Core::Template->new('html','test.html');
  -oder-
  $tpl = Sdoc::Core::Template->new('html',\$str);
...
sub key {
    ...
    if (ref $arg) {
        if (Sdoc::Core::Reference->isArrayRef($arg)) {
            return $arg->[0];
        }
        else {
            return $$arg;
...
sub value {
    ...
    elsif (Scalar::Util::blessed($arg) && $arg->isa('Sdoc::Core::Template')) {
        # Referenz auf Template-Objekt -> asString() aufrufen
        return $arg->asString;
    }
    elsif (Sdoc::Core::Reference->isArrayRef($arg)) {
