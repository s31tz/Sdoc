...
=head4 Synopsis
  ...
  @arr|$val = Sdoc::Core::Perl->perlDoFile($file);
...
Laden einer Konfigurationsdatei:
  ...
  %cfg = Sdoc::Core::Perl->perlDoFile($file);
...
sub packages {
    ...
    return wantarray? @arr: Sdoc::Core::Array->new(\@arr);
}
...
=head4 Example
  ...
  Sdoc::Core::Perl->classExists('Sdoc::Core::Object');
  ==>
  1
...
=head4 Example
  ...
  Sdoc::Core::Perl->loadClass('My::Application');
...
Der Aufruf Sdoc::Core::Perl->baseClasses('Pkg5') liefert ein Array
mit den Elementen
...
Der Aufruf Sdoc::Core::Perl->baseClassesISA('Pkg5') liefert ein Array
mit den Elementen
...
sub baseClassesISA {
    ...
    my (@arr,%seen);
    for (Sdoc::Core::Perl->hierarchyISA($class)) {
        push @arr,$_ if !$seen{$_}++;
    }
...
Der Aufruf Sdoc::Core::Perl->hierarchyISA('Pkg5') liefert ein Array
mit den Elementen
...
Der Aufruf Sdoc::Core::Perl->subClasses('Pkg1') liefert ein Array
mit den Elementen:
...
Liste in sortierter Form
  ...
  @arr = Sdoc::Core::Perl->subClasses('Pkg1')->sort;

=item *

Liste, eingeschränkt auf Klassen, deren Name einen Regex erfüllt:

  @arr = Sdoc::Core::Perl->subClasses('Pkg1')->select(qr/[45]/);
...
sub subClasses {
    ...
    return wantarray? @arr: Sdoc::Core::Array->bless(\@arr);
}
...
Alias für Subroutine aus anderer Klasse:
  ...
  Sdoc::Core::Perl->createAlias('MyClass',mySub=>\&MyClass1::mySub1);

=item *

Eintrag einer Closure in die Symboltabelle:

  Sdoc::Core::Perl->createAlias(__PACKAGE__,mySub=>sub { <code> });
...
=head4 Example
  ...
  $ref = Sdoc::Core::Perl->getHash($package,'H');
...
Setze Paket-Hash 'h' auf den Wert %hash:
  ...
  $ref = Sdoc::Core::Perl->setHash($package,'h',\%hash);
...
Erzeuge in $class das Array @A:
  ...
  $ref = Sdoc::Core::Perl->createArray($class,'A');

=item *

die Referenz kann geblesst werden:

  bless $ref,'Sdoc::Core::Array';
...
=head4 Example
  ...
  $ref = Sdoc::Core::Perl->getArray($package,'A');
...
Setze Paket-Array 'a' auf den Wert @arr:
  ...
  $ref = Sdoc::Core::Perl->setArray($package,'a',\@arr);
...
Setze Paket-Skalar 'n' auf den Wert 99:
  ...
  $ref = Sdoc::Core::Perl->setScalarValue($package,n=>99);
...
Ermittele Wert von Paket-Skalar 'n':
  ...
  $val = Sdoc::Core::Perl->getScalarValue($package,'n');
...
Skalar
  ...
  $ref = Sdoc::Core::Perl->setVar($package,'$','s',\99);

=item *

Array

  $ref = Sdoc::Core::Perl->setVar($package,'@','a',\@arr);

=item *

Hash

  $ref = Sdoc::Core::Perl->setVar($package,'%','h',\%hash);
...
sub setVar {
    ...
    my $type = Sdoc::Core::Perl->sigilToType($sigil);
...
Definition:
  ...
  $ref = Sdoc::Core::Perl->setSubroutine('My::Class',m=>sub {...});
