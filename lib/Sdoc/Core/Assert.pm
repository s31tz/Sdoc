...
Sdoc::Core::Assert - Zusicherungen
...
Test von der Kommandozeile aus:
  ...
  $ perl -MSdoc::Core::Assert -E 'Sdoc::Core::Assert->new->isNotNull("",-name=>"x")'
  Exception:
      ASSERT-00002: Value is null
  Name:
      x
  Stacktrace:
      Sdoc::Core::Assert::isNotNull() [+1 -e]
...
sub pathExists {
    ...
    if (!defined($path) || $path eq '' || Sdoc::Core::Path->exists($path)) {
        return 1;
    }
    elsif (defined wantarray) {
        return 0;
...
sub isDate {
    ...
    if (defined($val) && $val ne '') {
        local $@;
        eval {Sdoc::Core::Time->new(split /-/,$val)};
        if ($@) {
...
sub isNumber {
    ...
    if (!defined($val) || $val eq '' || Sdoc::Core::Math->isNumber($val)) {
        return 1;
    }
    elsif (defined wantarray) {
        return 0;
