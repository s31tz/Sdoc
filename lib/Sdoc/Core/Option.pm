...
Sdoc::Core::Option - Verarbeitung von Programm- und Methoden-Optionen
...
Instantiierung eines Options-Objekts:
    ...
    sub meth {
        ...
        my $opt = Sdoc::Core::Option->extract(\@_,
            -logLevel=>1,
            -verbose=>0,
        );
        ...
...
Das gelieferte Options-Objekt $opt ist eine Sdoc::Core::Hash-Instanz.
...
=head4 Example
    ...
    Sdoc::Core::Option->extractMulti(\@_,
        -select=>\@select,
        -from=>\@from,
        -where=>\@where,
        -limit=>\$limit,
