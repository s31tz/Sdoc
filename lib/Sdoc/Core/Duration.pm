...
Sdoc::Core::Duration - Rechnen und Konvertieren von Zeiträumen
...
=head4 Synopsis
    ...
    $dur = Sdoc::Core::Duration->new($sec);
    $dur = Sdoc::Core::Duration->new($str);
...
sub asShortString {
    ...
    if ($minUnit =~ /^[dhm]$/ && $arr[3]) {
        $arr[2] += Sdoc::Core::Math->roundToInt($arr[3]/60);
        $arr[3] = 0;
    }
    if ($minUnit =~ /^[dh]$/ && $arr[2]) {
        $arr[1] += Sdoc::Core::Math->roundToInt($arr[2]/60);
        $arr[2] = 0;
    }
    if ($minUnit eq 'd' && $arr[1]) {
        $arr[0] += Sdoc::Core::Math->roundToInt($arr[1]/24);
        $arr[1] = 0;
    }
...
Zeitdauer-Zeichenkette bestehend aus Tagen, Stunden, Mintuten, Sekunden:
    ...
    $sec = Sdoc::Core::Duration->stringToSeconds('152d5h25m3.457s');
    # 13152303.457

Dasselbe mit Doppelpunkt-Notation:

    $sec = Sdoc::Core::Duration->stringToSeconds('152:5:25:3.457');
    # 13152303.457
