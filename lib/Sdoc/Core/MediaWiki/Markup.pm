...
Sdoc::Core::MediaWiki::Markup - MediaWiki Code Generator
...
sub comment {
    ...
    my $code = Sdoc::Core::Unindent->trim($text);
    if ($code ne '') {
        if ($code =~ tr/\n//) {
            # Mehrzeiliger Kommentar
...
sub image {
    ...
    my $opt = Sdoc::Core::Parameters->extractPropertiesToObject(\@_,
        align => undef,
        alt => undef,
        border => undef,
        caption => undef,
...
sub item {
    ...
    my $val = Sdoc::Core::Unindent->trim(shift);
...
=head4 Example
  ...
  $ perl -MSdoc::Core::MediaWiki::Markup -C -E 'print Sdoc::Core::MediaWiki::Markup->testPage'
...
sub testPage {
    ...
    my $code .= $gen->comment(q~
      ...
      Sdoc::Core::MediaWiki::Markup implementierten Syntaxelemente
