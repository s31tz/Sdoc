...
Sdoc::Core::Parameters - Verarbeitung von Programm- und Methodenparametern
...
Optionen eine Exception wirft:
  ...
  sub parameters {
      ...
      my ($argA,$opt) = Sdoc::Core::Parameters->extract($varMode,$properties,
...
sub extract {
    ...
    while ($i < @$paramA) {
        ...
        if (!$properties && ($noMoreOptions || !defined($param) ||
            ...
            $key = Sdoc::Core::Converter->snakeCaseToCamelCase($key);
    ...
    return $varMode? \@args: (\@args,Sdoc::Core::Hash->new(\%opt));
}
