...
Sdoc::Core::Config - Konfigurationsdatei in "Perl Object Notation"
...
=head1 SYNOPSIS
    ...
    my $cfg = Sdoc::Core::Config->new('/etc/myapp/test.conf');
    my $database = $cgf->get('database');

=head1 DESCRIPTION

Ein Objekt der Klasse Sdoc::Core::Config repräsentiert eine Menge von
...
sub new {
    ...
    if (@_ == 1 && $_[0] =~ /=>/) { # "$key => $val, ..."
        ...
        if (!-e $cfgFile) {
            if (defined $create) {
                $create = Sdoc::Core::Unindent->trimNl($create);
                Sdoc::Core::Path->write($cfgFile,$create,-recursive=>1);
            }
            else {
        ...
        %cfg = Sdoc::Core::Perl->perlDoFile($cfgFile);
    }
...
sub try {
    ...
    for my $key (@_) {
        ...
        if (!ref $val && defined $val) {
            ...
            for my $key ($val =~ /%(\w+)%/g) {
                if ($key eq 'CWD') {
                    $val =~ s/%CWD%/Sdoc::Core::Process->cwd/e;
                }
                else {
