...
=head4 Example
  ...
  local $/ = Sdoc::Core::Path->newlineStr($file);
...
Es liegt noch keine Datei mit einer dreistelligen Nummer NNN vor:
  ...
  $num = Sdoc::Core::Path->nextNumber($dir,3);
  =>
  001
...
sub read {
    ...
    if ($decode) {
        ...
        $data = Sdoc::Core::String->autoDecode($data);
    }
...
Wende Sdoc::Core::Unindent->trimNl() auf die Daten $data an. Dies ist für
...
sub write {
    ...
    if ($encode) {
        Sdoc::Core::Perl->binmode(*F,":encoding($encode)");
    }
    ...
    if (defined($$ref) && $$ref ne '') {
        ...
        if ($unindent) {
            $$ref = Sdoc::Core::Unindent->trimNl($$ref);
        }
...
sub writeInline {
    ...
    $data = Sdoc::Core::Unindent->hereDoc($data);
...
sub tempDir {
    my $this = shift;
    return Sdoc::Core::TempDir->new(@_);
}
...
Innerhalb eines Verzeichnisses:
  ...
  $ perl -MSdoc::Core::Path -E 'Sdoc::Core::Path->moveToDateSubDir($_) for glob("*.jpg")'

Mit Zielverzeichnis ("dest"):

  $ perl -MSdoc::Core::Path -E 'Sdoc::Core::Path->moveToDateSubDir($_,"dest") for glob("src/*.jpg")'
...
$ perl -MSdoc::Core::Path -E 'Sdoc::Core::Path->partionizeDir($dir)'
...
Ausgangslage: Unterhalb von /tmp existieren weder a noch x.
  ...
  Sdoc::Core::Path->write($srcPath,'',-recursive=>1);
  Sdoc::Core::Path->rename($srcPath,$destPath,-recursive=>1);
...
sub rename {
    ...
    if ($recursive) {
        my $newDir = (Sdoc::Core::Path->split($newPath))[0];
        if ($newDir && !-d $newDir) {
    ...
    if ($recursive) {
        while (1) {
            ($oldPath) = Sdoc::Core::Path->split($oldPath);
            eval {Sdoc::Core::Path->rmdir($oldPath)};
            if ($@) {
                last;
            }
        }
...
=head4 Example
  ...
  $ perl -MSdoc::Core::Path -E 'Sdoc::Core::Path->numberBasePaths([$p->glob("*")],5,10)'
...
=head4 Example
  ...
  $ perl -MSdoc::Core::Path -E 'Sdoc::Core::Path->swap("06-album","0000940.*","0010200.*")'
...
=head4 Example
  ...
  Sdoc::Core::Path->symlinkRelative('a','x')
  # x => a
  
  Sdoc::Core::Path->symlinkRelative('a/b','x')
  # x => a/b
  
  Sdoc::Core::Path->symlinkRelative('a/b','x/y')
  # x/y => ../a/b
  
  Sdoc::Core::Path->symlinkRelative('a/b','x/y/z')
  # x/y/z => ../../a/b
