...
sub new {
    ...
    $str = Sdoc::Core::Unindent->trimNl($str);
...
=head3 asLaTeX() - Liefere LaTeX-Tabelle

=head4 Synopsis

    $code = $class->asLaTeX($ltx);
...
LaTeX Generator-Objekt (Sdoc::Core::LaTeX::Generator).

=back

=head4 Returns

LaTeX-Tabelle (String)

=head4 Description

Liefere die Tabelle als LaTeX-Code.
...
sub asLaTeX {
    ...
    if (@$titleA) {
        ...
        for (my $i = 0; $i < @$titleA; $i++) {
            ...
            my $title = $ltx->protect($titleA->[$i]);
            $title =~ s/\n/ \\\\ /g;
            $tmp .= '\textbf{'.$ltx->cmd('makecell',
                -o => $alignA->[$i].'b',
                -p => $title,
                -nl => 0,
            ).'}';
        ...
        $code .= "$tmp \\\\ \\hline\n";
        $code .= $ltx->cmd('endfirsthead');
    }
    $code .= $ltx->cmd('hline');
    $code .= $ltx->cmd('endlastfoot');

    for my $rowA ($self->rows) {
        my $tmp;
        for (my $i = 0; $i < @$rowA; $i++) {
            if ($tmp) {
                $tmp .= ' & ';
            }
            my $val = $ltx->protect($rowA->[$i]);
            $val =~ s/\n/ \\\\ /g;
            $tmp .= $ltx->cmd('makecell',
                -o => $alignA->[$i].'t',
                -p => $val,
                -nl => 0,
            );
    ...
    return $ltx->env('longtable',$code,
        -p => join('',$self->alignments),
    );
}
