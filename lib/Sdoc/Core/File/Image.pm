...
sub extension {
    ...
    return $self->memoize('extension',sub {
        my ($self,$key) = @_;
        return Sdoc::Core::Path->extension($self->path);
    });
}
...
sub type {
    ...
    return $self->memoize('type',sub {
        ...
        my $type = Sdoc::Core::Image->type($self->path);
        if ($type eq 'jpeg') {
            $type = 'jpg';
        }
...
sub scaleFactor {
    ...
    return Sdoc::Core::Array->min([$width/$self->width,$height/$self->height]);
}
...
sub aspectRatio {
    my $self = shift;
    return Sdoc::Core::Image->aspectRatio($self->width,$self->height);
}
...
Der Property-Hash ist ein Sdoc::Core::Hash-Objekt, dessen Schlüssel
