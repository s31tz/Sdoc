...
Sdoc::Core::TempFile - Temporäre Datei
...
sub new {
    ...
    my @args;
    if (defined $dir) {
        $dir = Sdoc::Core::Path->expandTilde($dir);
        push @args,'DIR',$dir;
    }
    if (defined $suffix) {
    ...
    if (defined $data) {
        Sdoc::Core::Path->write($self,$data);
    }
    if ($pathOnly) {
        Sdoc::Core::Path->delete("$self");
    }
