...
=item highlightStyle => $style (Default: 'default')
...
sub generateHtml {
    ...
    # Verbatim-Umgebung und Sourcecode-Highlighting
    ...
    if ($att->verbatim || $att->sourceCode) {
        ...
        if ($att->sourceCode) {
            $rules .= Sdoc::Core::Html::Pygments->css($doc->highlightStyle,
                '.sdoc-code-text');
        }
    }
...
sub generateLatex {
    ...
    if ($h->sourceCode) {
        ...
        push @preamble,
            ...
            $l->c('\usemintedstyle{%s}',$self->highlightStyle),
        ;
    }
    elsif ($h->verbatim) {
        push @packages,
