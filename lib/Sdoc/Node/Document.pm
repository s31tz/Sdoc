...
sub latex {
    ...
    my @opt = ($self->latexFontSize,$self->latexPaperSize);
    $code .= $l->c('\documentclass[%s]{%s}',\@opt,$documentClass,-nl=>2);
    ...
    for my $c ('a','b','c','d') { # Längen für Verbatim-Einrückung generieren
        ...
        for my $i ('','i') {
            ...
            $code .= $l->c('\newlength{%s}',$name);
            $code .= $l->c('\settowidth{%s}{\texttt{\scriptsize %s}}',
                $name,$n);
    ...
    $code .= $l->c('\hypersetup{%s}',\@opt,-nl=>2);
    ...
    if ($title || $author || $date) {
        ...
        $body .= $l->c('\author{%s}',$author);
        $body .= $l->c('\date{%s}',$date);
        $body .= $l->c('\maketitle',-nl=>2);
    }
    ...
    if (!$toc && $self->tableOfContents) {
        ...
        $body .= $self->latexTableOfContents($l);
    }
