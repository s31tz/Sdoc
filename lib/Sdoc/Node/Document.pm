...
Die Quelle der Eingabe (für Fehlermeldungen). Dies ist eine Zeichenkette
(Dateipfad) oder eine Referenz (String- oder Array-Referenz),
siehe Sdoc::Core::LineProcessor.
...
sub latex {
    ...
    # Dokumentklasse ermitteln

    my $documentType = $self->documentType;
    ...
    if (!$documentClass) {
        $self->throw(
            q~SDOC-00006: Unknown document type~,
    ...
    # SecNumDepth ermitteln

    my $secNumDepth = $self->sectionNumberDepth;
    if ($documentType eq 'book') {
        $secNumDepth -= 2;
    }
    elsif ($documentType eq 'report') {
        $secNumDepth -= 1;
    }
