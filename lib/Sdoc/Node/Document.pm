...
sub latex {
    ...
    return Sdoc::Core::LaTeX::Document->latex($l,
        ...
        packages => [
            microtype => 1, # autom. Korrektur von Leerraum
            #fancyhdr => $header, # bessere Kopf- und Fußzeilen
            ...
            lastpage => 1, # Ermittelung der letzten Seite
            hyperref => 1, # Verlinkung (laut Doku als letztes Paket laden)
            showframe => $showFrames, # Kennzeichnung Seitenbereiche
        ],
        preamble => [
            # geometry
            $geometry? $l->c('\geometry{%s}',$geometry): '',

            # Hilfsmakro \nolink für Unterdrückung eines Link
            $l->c('\newcommand*{\nolink}[1]{'.
            ...
            Sdoc::Core::Concat->catIf($documentClass =~ /^scr/,sub {
        ...
        body => [
            ...
            Sdoc::Core::Concat->catIf(!$toc && $self->tableOfContents,sub {
                $self->latexTableOfContents($l);
            }),
            # Code aller Kindknoten erzeugen
            $self->generateChilds('latex',$l),
        ],
    );
}
